    ...
    if (action === 'add-to-queue') {
        ...
        const handleOptionClick = async (e) => {
            ...
            if (option.classList.contains('create-new-option')) {
                ...
                document.getElementById('playlist-name-input').value = '';
                document.getElementById('playlist-cover-input').value = '';
                const descInput2 = document.getElementById('playlist-description-input');
...
export function initializeTrackInteractions(player, api, mainContent, contextMenu, lyricsManager, ui, scrobbler) {
    ...
    // Player bar 3-dot menu: open shared context menu for current track
    const playerMoreBtn = document.querySelector('.player-more-wrapper .detail-more-btn');
    if (playerMoreBtn) {
        playerMoreBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            e.preventDefault();

            if (!player.currentTrack) return;

            // Close any custom dropdown that might open
            document.querySelectorAll('.detail-more-dropdown.open').forEach((d) => d.classList.remove('open'));

            contextTrack = player.currentTrack;
            contextMenu._contextTrack = contextTrack;
            contextMenu._contextType = 'track';

            await updateContextMenuLikeState(contextMenu, contextTrack);

            // Position above the button (player bar is at the bottom)
            const rect = playerMoreBtn.getBoundingClientRect();
            positionMenu(contextMenu, rect.left, rect.top - 10, rect);
        });
    }
