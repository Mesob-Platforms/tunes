...
export const networkInfo = {
    // Returns 'slow-2g', '2g', '3g', or '4g' (default '4g' if unsupported)
    getEffectiveType() {
    ...
    isModerate() {
        return this.getEffectiveType() === '3g';
    },

    // Get recommended timeout based on network quality
    getTimeout(defaultMs = 6000) {
        if (this.isSlow()) return Math.min(defaultMs, 4000);
        if (this.isModerate()) return Math.min(defaultMs, 5000);
        return defaultMs;
    },

    // How many home sections to load (1 on very slow, 3 on fast)
    getHomeSectionCount() {
        if (this.isSlow()) return 1;       // Songs only
        if (this.isModerate()) return 2;   // Songs + albums
        return 3;                          // All sections
    },
};
...
export class LosslessAPI {
    ...
    async _doFetchWithRetry(relativePath, options = {}) {
    ...
    // --- Last.fm popularity helpers ---
    static LASTFM_API_KEY = '0ecf01914957b40c17030db822845a76';
    static LASTFM_API_URL = 'https://ws.audioscrobbler.com/2.0/';

    async getLastFmTrackPopularity(query, options = {}) {
        try {
            const url = `${LosslessAPI.LASTFM_API_URL}?method=track.search&track=${encodeURIComponent(query)}&api_key=${LosslessAPI.LASTFM_API_KEY}&format=json&limit=30`;
            ...
            for (const t of tracks) {
                ...
                if (!map.has(key) || listeners > map.get(key)) {
    ...
    async getLastFmArtistPopularity(query, options = {}) {
        try {
            const url = `${LosslessAPI.LASTFM_API_URL}?method=artist.search&artist=${encodeURIComponent(query)}&api_key=${LosslessAPI.LASTFM_API_KEY}&format=json&limit=30`;
            const response = await fetch(url, { signal: options.signal });
            const data = await response.json();
            ...
            for (const a of artists) {
                ...
                const listeners = parseInt(a.listeners, 10) || 0;
                if (!map.has(key) || listeners > map.get(key)) {
                    map.set(key, listeners);
                }
            }
            ...
            if (error.name === 'AbortError') throw error;
            console.error('Last.fm artist search failed:', error);
            return new Map();
        }
    }
    ...
    async getArtist(artistId) {
        ...
        // Detect variant/edition albums that should be pushed to the bottom
        const isVariant = (title) =>
            /\((Explicit|Deluxe|Video|Alternate|Alt|Clean|Edited|Remaster|Anniversary|Platinum|Special|Bonus|Extended|Complete|Collector|Limited|Japan|UK|International|Live|Acoustic|Instrumental|Stripped|Commentary|Tour|Expanded|Super Deluxe|Ultimate)[^)]*\)/i.test(title);

        const allReleases = this.deduplicateAlbums(rawReleases).sort((a, b) => {
            // Primary: non-variant albums first, variant albums last
            const aVariant = isVariant(a.title);
            const bVariant = isVariant(b.title);
